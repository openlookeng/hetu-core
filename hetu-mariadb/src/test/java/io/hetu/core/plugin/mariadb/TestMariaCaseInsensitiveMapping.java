...
public class TestMariaCaseInsensitiveMapping
        extends AbstractTestQueryFramework
{
    private final DockerizedMariaServer mariaServer;

    public TestMariaCaseInsensitiveMapping()
    {
        // Using dockerized Maria server instead of TestingMySqlServer because TestingMySqlServer depends on host's file system.
        ...
        this(new DockerizedMariaServer());
    }

    public TestMariaCaseInsensitiveMapping(DockerizedMariaServer mariaServer)
    {
        super(() -> createMariaQueryRunner(
                mariaServer,
                ImmutableMap.of("case-insensitive-name-matching", "true"),
                ImmutableList.of()));
        this.mariaServer = mariaServer;
    }

    private static QueryRunner createMariaQueryRunner(DockerizedMariaServer mariaServer, Map<String, String> connectorProperties, Iterable<TpchTable<?>> tables)
            throws Exception
    {
        try {
            mariaServer.init();
            return MariaQueryRunner.createMariaQueryRunner(mariaServer.getJdbcUrl(), connectorProperties, tables);
        }
        catch (Throwable e) {
            closeAllSuppress(e, mariaServer);
            throw e;
        }
    }
    ...
    {
        try (Connection connection = DriverManager.getConnection(mariaServer.getJdbcUrl());
