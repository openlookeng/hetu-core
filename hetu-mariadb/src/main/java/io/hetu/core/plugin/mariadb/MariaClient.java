...
import io.hetu.core.plugin.mariadb.optimization.MariaQueryGenerator;
        ...
        extends BaseJdbcClient
{
    ...
    @Inject
    public MariaClient(BaseJdbcConfig config, @StatsCollecting ConnectionFactory connectionFactory, TypeManager typeManager, ExternalFunctionHub externalFunctionHub)
    {
    ...
    protected Collection<String> listSchemas(Connection connection)
    {
        ...
        try (ResultSet resultSet = connection.getMetaData().getCatalogs()) {
            ...
            while (resultSet.next()) {
                ...
                // skip internal schemas of MairaDB(e.g. mysql schema)
    ...
    public void abortReadConnection(Connection connection)
            throws SQLException
    {
        // Abort connection before closing. Without this, the MariaDB driver
            ...
            throws SQLException
    {
        ...
        return metadata.getTables(schemaName.orElse(null), null, escapeNamePattern(tableName, escape).orElse(null), new String[] {"TABLE", "VIEW"});
    }
    ...
    {
        // In most cases, the running efficiency of MariaDB is not satisfactory, so just base push down by default.
        JdbcPushDownModule mariaPushDownModule = pushDownModule == DEFAULT ? BASE_PUSHDOWN : pushDownModule;
        JdbcPushDownParameter pushDownParameter = new JdbcPushDownParameter(getIdentifierQuote(), this.caseInsensitiveNameMatching, mariaPushDownModule, functionResolution);
        ...
        try (Connection connection = connectionFactory.openConnection(JdbcIdentity.from(session)); PreparedStatement statement = connection.prepareStatement(sql)) {
            ...
            if (metaData != null) {
                for (int i = 1; i <= metaData.getColumnCount(); i++) {
                    ...
                    // MariaDB JDBC returns decimal(41, 0) for output of SQL functions like sum,
                    ...
                    if (dataType == Types.DECIMAL && (precision > MAX_PRECISION || scale < 0)) {
                        // Covert MariaDB Decimal type to Presto type
