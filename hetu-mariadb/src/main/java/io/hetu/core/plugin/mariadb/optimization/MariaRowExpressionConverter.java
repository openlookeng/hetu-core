...
import io.hetu.core.plugin.mariadb.MariaConstants;
import io.hetu.core.plugin.mariadb.optimization.function.MariaApplyRemoteFunctionPushDown;
...
import static io.hetu.core.plugin.mariadb.optimization.MariaPushDownUtils.getCastExpression;
import static io.prestosql.spi.StandardErrorCode.NOT_SUPPORTED;
import static io.prestosql.spi.type.VarcharType.VARCHAR;
import static io.prestosql.sql.builder.functioncall.BaseFunctionUtil.isDefaultFunction;

public class MariaRowExpressionConverter
        extends BaseJdbcRowExpressionConverter
{
    private final MariaApplyRemoteFunctionPushDown mariaApplyRemoteFunctionPushDown;

    public MariaRowExpressionConverter(DeterminismEvaluator determinismEvaluator, RowExpressionService rowExpressionService, FunctionMetadataManager functionManager, StandardFunctionResolution functionResolution, BaseJdbcConfig baseJdbcConfig)
    {
        super(functionManager, functionResolution, rowExpressionService, determinismEvaluator);
        this.mariaApplyRemoteFunctionPushDown = new MariaApplyRemoteFunctionPushDown(baseJdbcConfig, MariaConstants.MARIA_CONNECTOR_NAME);
    }
    ...
    public String visitCall(CallExpression call, JdbcConverterContext context)
    {
        ...
        if (!isDefaultFunction(call)) {
            Optional<String> result = mariaApplyRemoteFunctionPushDown.rewriteRemoteFunction(call, this, context);
            if (result.isPresent()) {
                return result.get();
            }
            throw new PrestoException(NOT_SUPPORTED, String.format("MariaDB connector does not support remote function: %s.%s", call.getDisplayName(), call.getFunctionHandle().getFunctionNamespace()));
        }

        if (standardFunctionResolution.isArrayConstructor(functionHandle)) {
            throw new PrestoException(NOT_SUPPORTED, "MariaDB connector does not support array constructor");
        }
        if (standardFunctionResolution.isSubscriptFunction(functionHandle)) {
            throw new PrestoException(NOT_SUPPORTED, "MariaDB connector does not support subscript expression");
        }
