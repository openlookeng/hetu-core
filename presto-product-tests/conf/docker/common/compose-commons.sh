...
if [[ -z "${PRESTO_SERVER_DIR:-}" ]]; then
    ...
    PRESTO_SERVER_DIR="${PROJECT_ROOT}/hetu-server/target/hetu-server-${PRESTO_VERSION}/"
fi
...
if [[ -z "${PRESTO_JDBC_DRIVER_JAR:-}" ]]; then
    source "${PRODUCT_TESTS_ROOT}/target/classes/presto.env"
    PRESTO_JDBC_DRIVER_JAR="${PROJECT_ROOT}/presto-jdbc/target/hetu-jdbc-${PRESTO_VERSION}.jar"
fi
