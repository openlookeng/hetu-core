        ...
        tester().assertThat(new PruneCountAggregationOverScalar(getFunctionManager()))
                .on(p ->
                        p.aggregation((a) -> a
                                ...
                                .step(AggregationNode.Step.SINGLE)
                                .globalGrouping()
                                .source(p.values(ImmutableList.of(p.symbol("orderkey")), ImmutableList.of(constantExpressions(BIGINT, 1L))))))
                .matches(values(ImmutableMap.of("count_1", 0)));
    }
        ...
        tester().assertThat(new PruneCountAggregationOverScalar(getFunctionManager()))
                .on(p -> {
                    ...
                    AggregationNode inner = p.aggregation((a) -> a
                            ...
                            .source(
                                    p.project(
                                            Assignments.of(totalPrice, p.variable(totalPrice.getName(), DOUBLE)),
                                            p.tableScan(
                                                    new TableHandle(
