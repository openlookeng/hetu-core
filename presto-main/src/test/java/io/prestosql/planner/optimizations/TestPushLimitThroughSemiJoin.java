...
{
    @Test
    public void testPushLimitThroughSemiJoinEnabled()
    {
        PushLimitThroughSemiJoin rule = Mockito.mock(PushLimitThroughSemiJoin.class);
        ...
        MockLocalQueryRunner queryRunner = new MockLocalQueryRunner(ImmutableMap.of("push_limit_through_semi_join", "true"));
        queryRunner.init();
        queryRunner.createPlan("select * from lineitem where orderkey in (select orderkey from orders) limit 10", rule);

        Mockito.verify(rule, times(1)).apply(anyObject(), anyObject(), anyObject());
    }

    @Test
    public void testPushLimitThroughSemiJoinDisabled()
    {
        PushLimitThroughSemiJoin rule = Mockito.mock(PushLimitThroughSemiJoin.class);
        when(rule.getPattern()).thenReturn(limit());
        when(rule.apply(any(), any(), any())).thenReturn(Rule.Result.empty());
        ...
        MockLocalQueryRunner queryRunner = new MockLocalQueryRunner(ImmutableMap.of("push_limit_through_semi_join", "false"));
        queryRunner.init();
