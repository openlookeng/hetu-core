...
public class TestLogicalPlanner
{
    ...
    {
        List<PrestoWarning> warnings = createTestWarnings(1);
        assertPlannerWarnings(queryRunner, "CREATE TABLE TAB AS SELECT * FROM NATION", new HashMap<>(), Optional.of(ImmutableList.of(new TestWarningsRule(warnings))));
    }

    public static void assertPlannerWarnings(LocalQueryRunner queryRunner, @Language("SQL") String sql, Map<String, String> sessionProperties, Optional<List<Rule<?>>> rules)
    {
        ...
        List<WarningCode> warnings = warningCollector.getWarnings().stream()
                .map(PrestoWarning::getWarningCode)
                .collect(toImmutableList());
        assertEquals(warnings.get(0).getName(), "CTE_REUSE_NOT_SUPPORTED");
    }
