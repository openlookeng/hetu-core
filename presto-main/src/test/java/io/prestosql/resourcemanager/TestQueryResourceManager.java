...
public class TestQueryResourceManager
{
    ...
    private ResourceGroupManager mgr = mock(ResourceGroupManager.class);
    private ResourceGroupInfo grpInfo = mock(ResourceGroupInfo.class);
    ...
    public TestQueryResourceManager()
    {
        ...
        when(grpInfo.getSoftMemoryLimit()).thenReturn(DataSize.succinctBytes(5000));
        when(mgr.getResourceGroupInfo(any())).thenReturn(grpInfo);
        when(mgr.getCachedMemoryUsage(any())).thenReturn(Long.valueOf(0));
        when(mgr.getSoftReservedMemory(any())).thenReturn(Long.valueOf(2500));
        when(mgr.getSoftCpuLimit(any())).thenReturn(Long.valueOf(201 * 60 * 60000));
        when(mgr.getHardCpuLimit(any())).thenReturn(Long.valueOf(300 * 60 * 60000));

        queryResourceManager = new QueryResourceManager(queryId, null, new ResourceGroupId("test"), mgr, resourceUpdateListener);
    }
    ...
    {
        boolean result = queryResourceManager.setResourceLimit(DataSize.succinctBytes(3000),
                Duration.succinctDuration(10, TimeUnit.MINUTES),
                DataSize.succinctBytes(300));
        assertEquals(result, false);

        result = queryResourceManager.setResourceLimit(DataSize.succinctBytes(100),
                Duration.succinctDuration(300, TimeUnit.HOURS),
                DataSize.succinctBytes(300));
        assertEquals(result, false);

        result = queryResourceManager.setResourceLimit(DataSize.succinctBytes(100),
                Duration.succinctDuration(50, TimeUnit.MINUTES),
                DataSize.succinctBytes(300));
