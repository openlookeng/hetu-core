...
public class QueryResourceManagerService
{
    ...
    {
        /* Todo(Nitin K) add a resourceMonitor and resourcePlanner based on observed
    ...
    private void resourcePlanner()
    {
        ...
        /* Todo(Nitin K) add action handler for queries out of bound on resources, like:
         *   - Suspend - Resume
    ...
    public QueryResourceManager createQueryResourceManager(QueryId queryId, Session session)
    {
        return new QueryResourceManager(queryId, session, resourceUpdateListener);
    }
