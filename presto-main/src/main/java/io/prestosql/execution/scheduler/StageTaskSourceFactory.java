...
import static io.prestosql.SystemSessionProperties.getFaultTolerantExecutionDefaultTaskMemory;
import static io.prestosql.SystemSessionProperties.getFaultTolerantExecutionMaxTaskSplitCount;
import static io.prestosql.SystemSessionProperties.getFaultTolerantExecutionMinTaskSplitCount;
import static io.prestosql.SystemSessionProperties.getFaultTolerantExecutionTargetTaskInputSize;
import static io.prestosql.SystemSessionProperties.getFaultTolerantExecutionTargetTaskSplitCount;
import static io.prestosql.SystemSessionProperties.getFaultTolerantPreserveInputPartitionsInWriteStage;
        ...
        implements TaskSourceFactory
{
    ...
    public static class SingleDistributionTaskSource
            implements io.prestosql.execution.scheduler.TaskSource
    {
            ...
            implements TaskSource
    {
                ...
                Executor executor)
        {
            ...
            Map<PlanNodeId, SplitSource> hashDistributionSplitSources = splitSourceFactory.createSplitSources(session, fragment);

            return new HashDistributionTaskSource(
                    hashDistributionSplitSources,
                    ...
                    (preserveInputPartitionsInWriteStage && isWriteFragment(fragment)) ? new DataSize(0, BYTE) : targetPartitionSourceSize,
                    getFaultTolerantExecutionDefaultTaskMemory(session),
                    executor);
        }
        ...
        {
            PlanVisitor<Boolean, Void> visitor = new InternalPlanVisitor<Boolean, Void>()
            {
                @Override
                    ...
                    loadedSplitsList -> {
                        synchronized (this) {
                            ...
                            for (LoadedSplits loadedSplits : loadedSplitsList) {
                                BucketNodeMap nodeMap = this.bucketNodeMap
                                        .orElseThrow(() -> new VerifyException("bucket to node map is expected to be present"));
                                for (Split split : loadedSplits.getSplits()) {
                                    int bucket = nodeMap.getBucket(split);
                                    int partition = getPartitionForBucket(bucket);

                                    if (!nodeMap.isDynamic()) {
                                        HostAddress requiredAddress = nodeMap.getAssignedNode(split).get().getHostAndPort();
            ...
            implements TaskSource
    {
        ...
        private boolean closed;
        @GuardedBy("this")
        private ListenableFuture<SplitSource.SplitBatch> currentSplitBatchFuture = immediateFuture(null);
                ...
                Executor executor)
        {
            ...
            PlanNodeId partitionedSrcNodeId = getOnlyElement(fragment.getPartitionedSources());
            Map<PlanNodeId, SplitSource> splitSources = splitSourceFactory.createSplitSources(session, fragment);
            SplitSource sourceSplit = splitSources.get(partitionedSrcNodeId);

            Optional<CatalogName> catalogName = Optional.of(sourceSplit.getCatalogName())
                    .filter(catalog -> !isInternalSystemConnector(catalog));

            return new SourceDistributionTaskSource(
                    session.getQueryId(),
                    partitionedSrcNodeId,
                    tableExecuteContextManager,
                    sourceSplit,
                    getReplicatedExchangeSourceHandles(fragment, exchangeSourceHandles),
                    splitBatchSize,
            ...
            extends AbstractFuture<LoadedSplits>
    {
        ...
        @GuardedBy("this")
        private ListenableFuture<SplitSource.SplitBatch> currentSplitBatch = immediateFuture(null);
        ...
        public synchronized void load()
        {
            ...
            Futures.addCallback(
                    currentSplitBatch,
                    new FutureCallback<SplitSource.SplitBatch>()
                    {
                        @Override
