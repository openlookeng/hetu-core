...
public final class MetadataUtil
{
    ...
    public static CatalogSchemaName createCatalogSchemaName(Session session, Node node, Optional<QualifiedName> schema)
    {
        ...
        if (schema.isPresent()) {
            ...
            // Hetu supports cluster so that there can be 3 dots in <cluster>.<catalog>.<schema>
            if (parts.size() > 3) {
                throw new SemanticException(INVALID_SCHEMA_NAME, node, "Too many parts in schema name: %s", schema.get());
            }
            // If the schema name has 3 dots, combine the first two components into one and use it as catalog
            if (parts.size() == 3) {
                catalogName = parts.get(0) + "." + parts.get(1);
            }
    ...
    public static QualifiedObjectName createQualifiedObjectName(Session session, Node node, QualifiedName name)
    {
        ...
        if (parts.size() > 3) {
            catalogName = (parts.size() > 3) ? parts.get(3) + "." + parts.get(2) : session.getCatalog().orElseThrow(() ->
                    new SemanticException(CATALOG_NOT_SPECIFIED, node, "Catalog must be specified when session catalog is not set"));
        }
        else {
            catalogName = (parts.size() > 2) ? parts.get(2) : session.getCatalog().orElseThrow(() ->
                    new SemanticException(CATALOG_NOT_SPECIFIED, node, "Catalog must be specified when session catalog is not set"));
        }
