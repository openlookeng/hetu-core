        ...
        extends SqlQueryExecution
{
    ...
    public CachedSqlQueryExecution(QueryPreparer.PreparedQuery preparedQuery, QueryStateMachine stateMachine,
                                   ...
                                   Optional<Cache<CachedDataKey, CachedDataStorage>> dataCache)
    {
            ...
            CostCalculator costCalculator, WarningCollector warningCollector, CachedDataStorageProvider cachedData)
    {
        ...
        boolean isDataCacheable = this.dataCache.isPresent() && cacheable;
        boolean isCteDataCacheable = this.dataCache.isPresent() && isCTEResultCacheEnabled(session) && isCTEReuseEnabled(session);
        ...
        CachedDataStorageProvider cachedDataStorageProvider = new CachedDataStorageProvider()
        {
            ...
            public CachedDataStorage getOrCreateCachedDataKey(CachedDataKey cachedDataKey)
            {
                ...
                CachedDataStorage cds = dataCache.get().getIfPresent(cachedDataKey); // Todo(Nitin) :
                if (cds == null) {
                    cds = new CachedDataStorage(cachedDataKey, new CatalogSchemaTableName(getDataCacheCatalogName(session), getDataCacheSchemaName(session), UUID.randomUUID().toString().replaceAll("-", "_")),
                            ...
                            null);
                    dataCache.get().put(cachedDataKey, cds);
                    CachedDataStorage finalCds = cds;
                    CachedDataKey finalCachedDataKey = cachedDataKey;
                    addStateChangeListener(newState -> {
                        if (newState == QueryState.FINISHED) {
                            finalCds.commit(System.currentTimeMillis(), 0);
                        }
                        if (newState == QueryState.FAILED) {
                            finalCds.abort();
                            dataCache.get().invalidate(finalCachedDataKey);
                        }
                    });
                    return cds;
                }
        ...
        // if (isResultCacheEnabled(session) && analysis.getStatement() instanceof Query) {
            try {
