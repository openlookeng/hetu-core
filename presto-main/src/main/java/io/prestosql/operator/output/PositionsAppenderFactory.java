...
import io.prestosql.operator.output.PositionsAppender.TypedPositionsAppender;
...
import io.prestosql.sql.gen.IsolatedClass;
...
{
    private final LoadingCache<CacheKey, PositionsAppender> cache;

    public PositionsAppenderFactory()
    {
        this.cache = CacheBuilder.newBuilder().maximumSize(1000).build(CacheLoader.from(key -> createAppender(key.type)));
    }
