...
public final class JsonOperators
{
    ...
    public static Long castToSmallint(@SqlType(JSON) Slice json)
    {
        try (JsonParser parser = createJsonParser(JSON_FACTORY, json)) {
            ...
            throw new PrestoException(INVALID_CAST_ARGUMENT, String.format("Cannot cast '%s' to %s", json.toStringUtf8(), SMALLINT), e);
        }
    }
    ...
    public static Slice castFromVarchar(@SqlType("varchar(x)") Slice value)
    {
        try {
            ...
            throw new PrestoException(INVALID_CAST_ARGUMENT, String.format("Cannot cast '%s' to %s", value.toStringUtf8(), JSON));
        }
    }
    ...
    private static Slice internalCastFromLong(long value, int estimatedSize)
    {
        try {
            ...
            throw new PrestoException(INVALID_CAST_ARGUMENT, String.format("Cannot cast '%s' to %s", value, JSON));
        }
    }
