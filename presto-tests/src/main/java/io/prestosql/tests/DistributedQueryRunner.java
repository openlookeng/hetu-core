        ...
        implements QueryRunner
{
    ...
    protected static final String ENVIRONMENT = "testing";
    protected static final SqlParserOptions DEFAULT_SQL_PARSER_OPTIONS = new SqlParserOptions();

    protected final TestingDiscoveryServer discoveryServer;
    protected final TestingPrestoServer coordinator;
    protected final List<TestingPrestoServer> servers;

    protected final Closer closer = Closer.create();

    protected final TestingPrestoClient prestoClient;
    ...
    public void createCatalog(String catalogName, String connectorName, Map<String, String> properties)
    {
        ...
        start = System.nanoTime();
        waitForCatalogAnnouncement(catalog, catalogName, start);
    }

    /**
     * Create Hetu's data center dynamic catalogs using the given properties.
     *
     * @param catalogName the parent catalog name
     * @param connectorName the connector name
     * @param properties the catalog properties
     */
    public void createDCCatalog(String catalogName, String connectorName, Map<String, String> properties)
    {
        long start = System.nanoTime();
        Set<CatalogName> catalogNames = new HashSet<>();
        for (TestingPrestoServer server : servers) {
            catalogNames.addAll(server.createDCCatalog(catalogName, connectorName, properties));
        }
        log.info("Created catalog %s (%s) in %s", catalogName, catalogNames.stream().map(CatalogName::getCatalogName).collect(Collectors.joining(",")), nanosSince(start));

        // wait for all nodes to announce the new catalog
        start = System.nanoTime();
        for (CatalogName catalog : catalogNames) {
            waitForCatalogAnnouncement(catalog, catalogName, start);
        }
    }

    private void waitForCatalogAnnouncement(CatalogName catalog, String catalogName, long start)
    {
    ...
    public static class Builder
    {
        ...
        public DistributedQueryRunnerWithStateStore buildWithStateStore()
                throws Exception
        {
            return new DistributedQueryRunnerWithStateStore(defaultSession, nodeCount, extraProperties, coordinatorProperties, parserOptions, environment, baseDataDir);
        }
    }
}
