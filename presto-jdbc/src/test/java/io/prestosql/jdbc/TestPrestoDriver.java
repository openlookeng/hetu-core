...
public class TestPrestoDriver
{
    ...
    @Test
    public void testTypesWithTimeZone()
            throws Exception
    {
    ...
    {
        try (Connection connection = createConnection()) {
            try (ResultSet rs = connection.getMetaData().getPrimaryKeys("blackhole", "blackhole", "test_table")) {
                assertPrimaryKeysMetadata(rs);
                assertFalse(rs.next());
            }
        }
    }

    private static void assertPrimaryKeysMetadata(ResultSet rs)
            throws SQLException
    {
        ResultSetMetaData metadata = rs.getMetaData();
        ...
        assertEquals(metadata.getColumnLabel(5), "KEY_SEQ");
        assertEquals(metadata.getColumnType(5), Types.SMALLINT);

        assertEquals(metadata.getColumnLabel(6), "PK_NAME");
        assertEquals(metadata.getColumnType(6), Types.VARCHAR);
    }
    ...
    {
        try (Connection connection = createConnection()) {
            try (ResultSet rs = connection.getMetaData().getImportedKeys("blackhole", "blackhole", "test_table")) {
                assertImportedAndExportedKeysMetadata(rs);
                assertFalse(rs.next());
            }
        }
    }
    ...
    {
        try (Connection connection = createConnection()) {
            try (ResultSet rs = connection.getMetaData().getExportedKeys("blackhole", "blackhole", "test_table")) {
                assertImportedAndExportedKeysMetadata(rs);
                assertFalse(rs.next());
            }
        }
    ...
    private static void assertImportedAndExportedKeysMetadata(ResultSet rs)
            throws SQLException
    {
        ResultSetMetaData metadata = rs.getMetaData();
        assertEquals(metadata.getColumnCount(), 14);

        assertEquals(metadata.getColumnLabel(1), "PKTABLE_CAT");
        assertEquals(metadata.getColumnType(1), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(2), "PKTABLE_SCHEM");
        assertEquals(metadata.getColumnType(2), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(3), "PKTABLE_NAME");
        assertEquals(metadata.getColumnType(3), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(4), "PKCOLUMN_NAME");
        assertEquals(metadata.getColumnType(4), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(5), "FKTABLE_CAT");
        assertEquals(metadata.getColumnType(5), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(6), "FKTABLE_SCHEM");
        assertEquals(metadata.getColumnType(6), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(7), "FKTABLE_NAME");
        assertEquals(metadata.getColumnType(7), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(8), "FKCOLUMN_NAME");
        assertEquals(metadata.getColumnType(8), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(9), "KEY_SEQ");
        assertEquals(metadata.getColumnType(9), Types.SMALLINT);

        assertEquals(metadata.getColumnLabel(10), "UPDATE_RULE");
        assertEquals(metadata.getColumnType(10), Types.SMALLINT);

        assertEquals(metadata.getColumnLabel(11), "DELETE_RULE");
        assertEquals(metadata.getColumnType(11), Types.SMALLINT);

        assertEquals(metadata.getColumnLabel(12), "FK_NAME");
        assertEquals(metadata.getColumnType(12), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(13), "PK_NAME");
        assertEquals(metadata.getColumnType(13), Types.VARCHAR);

        assertEquals(metadata.getColumnLabel(14), "DEFERRABILITY");
        assertEquals(metadata.getColumnType(14), Types.SMALLINT);
    }
