...
public class TestJdbcResultSet
{
    ...
    public void testGetBinaryStream()
            throws Exception
    {
        try (ResultSet rs1 = statement.executeQuery("SELECT cast('123' as varbinary) x")) {
            rs1.next();
            InputStream inputStream = rs1.getBinaryStream(1);
            byte[] bytes;
            try {
        ...
        try (ResultSet rs2 = statement.executeQuery("SELECT cast('123' as varbinary) x")) {
            rs2.next();
            InputStream inputStream = rs2.getBinaryStream("x");
            byte[] bytes;
            try {
                bytes = new byte[inputStream.available()];
                inputStream.read(bytes);
        ...
        try (ResultSet rs3 = statement.executeQuery("SELECT NULL x")) {
            rs3.next();
            InputStream inputStream = rs3.getBinaryStream("x");
            assertNull(inputStream);
        }

        try (ResultSet rs4 = statement.executeQuery("SELECT NULL x")) {
            rs4.next();
            InputStream inputStream = rs4.getBinaryStream(1);
            assertNull(inputStream);
        }
    }
