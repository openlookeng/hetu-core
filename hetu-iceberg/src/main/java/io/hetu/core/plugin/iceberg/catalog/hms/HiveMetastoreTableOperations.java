    ...
    protected void commitToExistingTable(TableMetadata base, TableMetadata metadata)
    {
        ...
        try {
            ...
            try {
                ...
                throw new PrestoException(ICEBERG_COMMIT_ERROR, format("Failed to commit to table %s.%s", database, tableName), e);
            }

            // todo privileges should not be replaced for an alter
            PrincipalPrivileges privileges = Optional.ofNullable(MetastoreUtil.buildInitialPrivilegeSet(table.getOwner())).orElse(NO_PRIVILEGES);
