...
public class HiveColumnProjectionInfo
{
    ...
    public long getRetainedSizeInBytes()
    {
        return INSTANCE_SIZE
                + SizeOfUtil.estimatedSizeOf(dereferenceIndices, SizeOfUtil::sizeOf)
                + SizeOfUtil.estimatedSizeOf(dereferenceNames, SizeOfUtil::estimatedSizeOf)
                + hiveType.getRetainedSizeInBytes()
                // type is not accounted for as the instances are cached (by TypeRegistry) and shared
                + SizeOfUtil.estimatedSizeOf(partialName);
    }
}
