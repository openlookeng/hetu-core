...
public final class HiveWriteUtils
{
    ...
    public static Serializer initializeSerializer(Configuration conf, Properties properties, String serializerName)
    {
        try {
            ...
            throw new PrestoException(HiveErrorCode.HIVE_SERDE_NOT_FOUND, "Serializer does not exist: " + serializerName);
        }
    ...
    public static List<String> createPartitionValues(List<Type> partitionColumnTypes, Page partitionColumns, int position)
    {
        ...
        for (int field = 0; field < partitionColumns.getChannelCount(); field++) {
            ...
            if (value == null) {
                partitionValues.add(HivePartitionKey.HIVE_DEFAULT_DYNAMIC_PARTITION);
            }
            else {
                String valueString = value.toString();
                if (!CharMatcher.inRange((char) 0x20, (char) 0x7E).matchesAllOf(valueString)) {
                    throw new PrestoException(HiveErrorCode.HIVE_INVALID_PARTITION_VALUE,
            ...
            HiveACIDWriteType writeType)
    {
        ...
        if (writeType == HiveACIDWriteType.UPDATE) {
            ...
            else if (AcidUtils.isInsertOnlyTable(parameters)) {
                throw new PrestoException(NOT_SUPPORTED, "Updates to Hive InsertOnly-transactional tables are not supported: " + tableName);
            }
        }
        ...
        if (HiveACIDWriteType.VACUUM_UNIFY == writeType &&
    ...
    public static Path getTableDefaultLocation(Database database, HdfsContext context, HdfsEnvironment hdfsEnvironment, String schemaName, String tableName)
    {
        ...
        if (!location.isPresent() || location.get().isEmpty()) {
            throw new PrestoException(HiveErrorCode.HIVE_DATABASE_LOCATION_ERROR, format("Database '%s' location is not set", schemaName));
        }
        ...
        if (!isS3FileSystem(context, hdfsEnvironment, databasePath)) {
            if (!pathExists(context, hdfsEnvironment, databasePath)) {
                throw new PrestoException(HiveErrorCode.HIVE_DATABASE_LOCATION_ERROR, format("Database '%s' location does not exist: %s", schemaName, databasePath));
            }
            if (!isDirectory(context, hdfsEnvironment, databasePath)) {
                throw new PrestoException(HiveErrorCode.HIVE_DATABASE_LOCATION_ERROR, format("Database '%s' location is not a directory: %s", schemaName, databasePath));
            }
        }
    ...
    public static boolean isHdfsEncrypted(HdfsContext context, HdfsEnvironment hdfsEnvironment, Path path)
    {
        try {
            ...
            throw new PrestoException(HiveErrorCode.HIVE_FILESYSTEM_ERROR, "Failed checking encryption status for path: " + path, e);
        }
    }
    ...
    public static Path createTemporaryPath(ConnectorSession session, HdfsContext context, HdfsEnvironment hdfsEnvironment, Path targetPath, OpertionType type)
    {
        ...
        if (isViewFileSystem(context, hdfsEnvironment, targetPath)) {
            ...
            stagingDirectoryName = randomUUID().toString();
        }
        else {
            temporaryPrefix = ((type != OpertionType.INSERT) ? targetPath.getParent() : targetPath).toString();
            if ((type == OpertionType.INSERT) && !pathExists(temporaryPrefix, context, hdfsEnvironment)) {
                temporaryPrefix = targetPath.getParent().toString();
            }
            String queryId = context.getQueryId().isPresent() ? context.getQueryId().get() : "";
            stagingDirectoryName = ".staging-" + queryId + "-" + randomUUID().toString();
        }

        // create a temporary directory on the same filesystem
        Path temporaryRoot = new Path(targetPath, temporaryPrefix);
        Path temporaryPath = new Path(temporaryRoot, stagingDirectoryName);

        createDirectory(context, hdfsEnvironment, temporaryPath);
        return temporaryPath;
    }

    private static boolean pathExists(String location, HdfsContext context, HdfsEnvironment hdfsEnvironment)
    {
        Path path = new Path(location);
        try {
            if (hdfsEnvironment.getFileSystem(context, path).exists(path)) {
                return true;
            }
        }
        catch (IOException e) {
