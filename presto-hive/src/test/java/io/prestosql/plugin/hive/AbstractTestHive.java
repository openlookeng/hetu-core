...
public abstract class AbstractTestHive
{
    ...
    protected final void setup(String databaseName, HiveConfig hiveConfig, HiveMetastore hiveMetastore)
    {
        ...
        pageSourceProvider = new HivePageSourceProvider(hiveConfig, hdfsEnvironment, getDefaultHiveRecordCursorProvider(hiveConfig), getDefaultHiveDataStreamFactories(hiveConfig), TYPE_MANAGER, getNoOpIndexCache(), null);
    }
    ...
    public void testTableCreationIgnoreExisting()
    {
        ...
        try {
            try (Transaction transaction = newTransaction()) {
                ...
                LocationHandle locationHandle = locationService.forNewTable(transaction.getMetastore(schemaName), session, schemaName, tableName, Optional.empty(), Optional.empty());
