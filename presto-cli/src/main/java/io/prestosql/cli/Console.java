...
public class Console
{
    ...
    private static void runConsole(QueryRunner queryRunner, AtomicBoolean exiting)
    {
        try (TableNameCompleter tableNameCompleter = new TableNameCompleter(queryRunner);
                LineReader reader = new LineReader(getHistory(), Completion.commandCompleter(), Completion.lowerCaseCommandCompleter(), tableNameCompleter)) {
            ...
            PrintStream errorChannel)
    {
        String finalSql;
        try {
            finalSql = QueryPreprocessor.preprocessQuery(
    ...
    {
        String historyFilePath = System.getenv("OPENLOOKENG_HISTORY_FILE");
        File historyFile;
        if (isNullOrEmpty(historyFilePath)) {
            historyFile = new File(getUserHome(), ".openLooKeng_history");
        }
        else {
